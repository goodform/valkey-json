...
Since there does not exist a standard for path syntax, ValkeyJSON implements its own. ValkeyJSON's syntax is a subset of common best practices and resembles [JSONPath](http://goessner.net/articles/JsonPath/) not by accident.

Paths always begin at the root of a ValkeyJSON value. The root is denoted by the period character (`.`). For paths referencing the root's children, prefixing the path with the root is optional.
...
By definition, a JSON key can be any valid JSON String. Paths, on the other hand, are traditionally based on JavaScript's (and in Java in turn) variable naming conventions. Therefore, while it is possible to have ValkeyJSON store objects containing arbitrary key names, accessing these keys via a path will only be possible if they respect these naming syntax rules:
